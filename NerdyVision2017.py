...
"""2017 FRC Vision with Microsoft Lifecam"""
__author__ = "Ted Lin"
...
def main():
    ...
    while 687:
        ...
        # for sample image testing
        '''
        frame = cv2.imread("SampleImages/" + str(sample_image) + ".jpg")
        print(sample_image)
        LOWER_LIM = SAMPLE_LOWER
        UPPER_LIM = SAMPLE_UPPER
        '''

        # for computing FPS information
        '''
        time_end = time.time()
        ...
        if shooting:
            ...
            # only proceed if at least two contours (two blocks around peg) are found
            if len(cnts) > 1:
                centers_x = [0]
                centers_y = [0]
